...
int main(int argc, char **argv)
{
    ...
    //             std::cerr << e.what() << '\n';
    //             connect(sockfd, (struct sockaddr *)&server_addr, sizeof(server_addr));
    //             ROS_INFO("Connected Sever again !!!");
    //             connect_flag = false;
    //             break;
    //         }
    //     }
    ...
    */
    ...
    while (ros::ok())
    {
        ...
        while (connect_flag)
        {                                                 // 创建TCP套接字
            sockfd = socket(AF_INET, SOCK_STREAM, 0); // 面向连接或者流格式套接字
            if (sockfd == -1)
            {
            ...
            server_addr.sin_port = htons(30005);                      // 服务器端口
            server_addr.sin_addr.s_addr = inet_addr("172.31.1.104"); // 服务器IP地址
            ...
            if (connect(sockfd, (struct sockaddr *)&server_addr, sizeof(server_addr)) == -1)
            {
                ...
                close(sockfd);
                //sleep(1);
                // std::cout<<connect_flag<<std::endl;
                connect_flag = true;
            }
            else
            {
